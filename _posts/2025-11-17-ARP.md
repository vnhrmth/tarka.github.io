...
DHCP server -> assigns IP Address to every device
mDNS query -> gets every IP Address in the network for whichever device that listens to the query and responds
ARP(OS network stack) -> does this IP address have a MAC address? -> NO -> sends a ARP Query -> other device returns the MAC address which is stored in sender's ARP cache
